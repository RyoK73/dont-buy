...
画面下部の入力ドック（記帳フォーム・商品登録フォーム）は紙面から独立した「ドック」として扱い、`neutral-paper` と区別される背景（`neutral-paper-line` またはやや濃いトーン）と `primary` の境界線で紙面から切り離す。左端には `secondary` 色のマージンラインを全画面共通の装飾として配置する。
